...
//answer hard
//https://discuss.leetcode.com/topic/65439/an-accepted-java-solution-detailed-explanation-hashmap-double-linked-list/2
...
class AllOne {
	...
	public void inc(String key) {
		if (tail == null) { // which means head must be null too and the keyMap
							// must be empty.
			curNode = new valueNode(1, key);
			head = curNode;
			tail = curNode;
			keyMap.put(key, curNode);
		} else if (!keyMap.containsKey(key)) { // which means that this key
												// should be add to the tail.
			if (tail.value == 1) {// which means just add the key to the tail
									// list.
			...
			if (curNode.preNode != null) { // which means the node is in the
											// middle.
	...
	public void dec(String key) {
		...
		curNode = keyMap.get(key);
		if (curNode.nextNode != null) { // which means the node is in the
										// middle.
			if (curNode.nextNode.value == curNode.value - 1) { // which means we
																// can just
