...
// https://discuss.leetcode.com/topic/2031/challenge-me-thx/41
/*
 ...
 * 
 * int singleNumber(int A[], int n) { int na = 0, nb = 0, nc = 0; for(int i = 0;
 * i < n; i++){ nb = nb ^ (A[i] & na); na = (na ^ A[i]) & ~nc; nc = nc ^ (A[i] &
 * ~na & ~nb); } return na & ~nb & ~nc; } Or even like this:
 * 
 * int singleNumber(int A[], int n) { int twos = 0xffffffff, threes =
 * 0xffffffff, ones = 0; for(int i = 0; i < n; i++){ threes = threes ^ (A[i] &
 * twos); twos = (twos ^ A[i]) & ~ones; ones = ones ^ (A[i] & ~twos & ~threes);
 * } return ones; } I hope all these above can help you have a better understand
 * of this problem.
 */