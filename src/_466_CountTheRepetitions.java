...
public class _466_CountTheRepetitions {
	public int getMaxRepetitions(String s1, int n1, String s2, int n2) {
		if (!ableToObtain(s1, s2))
			return 0; // check if [s1. ∞] obtains s2
		int cnt = 0, k = -1;
		String s = s1;
		StringBuilder remainBuilder; // record `remain string`
		ArrayList<String> stringList = new ArrayList<>(); // record all the
															// `remain string`
		ArrayList<Integer> countList = new ArrayList<>(); // record matching
															// count from start
															// to the current
															// remain string
		...
		for (int i = 0; i <= n1; i++) {
			...
			cnt += getRemain(s, s2, remainBuilder); // get the next remain
													// string, returns the count
													// of matching
			String remain = remainBuilder.toString();
			if ((k = stringList.indexOf(remain)) != -1)
				break; // if there is a loop, break
		...
		int countOfLoop = cnt - countList.get(k), loopLength = stringList
				.size() - k; // get matching count in the loop, and loop length
		cnt = countList.get(k);
		n1 -= k;
